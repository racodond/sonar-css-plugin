...
public class CompoundSelectorTreeTest extends ScssTreeTest {
  ...
  public void compoundSelector() {
    ...
    tree = checkParsed("&::after");
    assertThat(tree.selectors()).hasSize(2);
    assertThat(tree.selectors().get(0)).isInstanceOf(ScssParentSelectorTree.class);
    ...
    tree = checkParsed("%foo");
    assertThat(tree.selectors()).hasSize(1);
    assertThat(tree.selectors().get(0)).isInstanceOf(ScssPlaceholderSelectorTree.class);

    tree = checkParsed("a%foo");
    assertThat(tree.selectors()).hasSize(2);
    assertThat(tree.selectors().get(0)).isInstanceOf(TypeSelectorTree.class);
    assertThat(tree.selectors().get(1)).isInstanceOf(ScssPlaceholderSelectorTree.class);

    tree = checkParsed(".class%foo");
    assertThat(tree.selectors()).hasSize(2);
    assertThat(tree.selectors().get(0)).isInstanceOf(ClassSelectorTree.class);
    assertThat(tree.selectors().get(1)).isInstanceOf(ScssPlaceholderSelectorTree.class);
  }
