...
public class ScssAnalyzerSensorTest {

  private final File baseDir = new File("src/test/resources/scss");
  ...
  public void should_execute_and_compute_valid_measures() {
    String relativePath = "measures.scss";
  ...
  public void should_execute_and_compute_valid_measures_on_file_starting_with_BOM() {
    String relativePath = "measuresWithBOM.scss";
    inputFile(relativePath);
    createScssSquidSensor().execute(context);
    assertMeasures("moduleKey:" + relativePath);
  }
  ...
  public void should_execute_and_save_issues_file_not_starting_with_BOM() {
    should_execute_and_save_issues("issues.scss");
  }
  ...
  public void should_execute_and_save_issues_file_starting_with_BOM() {
    should_execute_and_save_issues("issuesWithBOM.scss");
  }
  ...
  private ScssAnalyzerSensor createScssSquidSensor() {
    return new ScssAnalyzerSensor(context.fileSystem(), checkFactory, settings, new NoSonarFilter());
  }
