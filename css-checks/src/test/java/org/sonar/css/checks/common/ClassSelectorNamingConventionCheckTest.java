...
public class ClassSelectorNamingConventionCheckTest {

  private final ClassSelectorNamingConventionCheck check = new ClassSelectorNamingConventionCheck();
  ...
  public void should_throw_an_illegal_state_exception_as_the_format_parameter_is_not_valid() {
    try {
      ...
      assertThat(e.getMessage()).isEqualTo("Check css:class-selector-naming-convention (Class selectors should follow a naming convention): "
        + "format parameter \"(\" is not a valid regular expression.");
    }
  }

  private File getTestFile(String fileName) {
    return CheckTestUtils.getCommonTestFile("class-selector-naming-convention/" + fileName);
  }
