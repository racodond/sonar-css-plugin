...
@Rule(
  key = "quoted-generic-family-names",
  name = "Generic family names should not be quoted",
  priority = Priority.CRITICAL,
  tags = {Tags.BUG})
@SqaleConstantRemediation("20min")
@ActivatedByDefault
public class QuotedGenericFamilyNamesCheck extends DoubleDispatchVisitorCheck {

  private static final List<String> GENERIC_FAMILY_NAMES = ImmutableList.of("serif", "sans-serif", "cursive", "fantasy", "monospace");
  ...
  public void visitPropertyDeclaration(PropertyDeclarationTree tree) {
    if (tree.property().standardProperty() instanceof FontFamily
      ...
      getAllStrings(tree.value()).stream()
        .filter(t -> GENERIC_FAMILY_NAMES.contains(t.actualText().toLowerCase()))
        .forEach(t -> addPreciseIssue(t, "Unquote this generic family name."));
    }
