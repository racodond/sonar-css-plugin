...
public class CounterValidator implements PropertyValueValidator {
  ...
  public boolean isValid(@Nonnull CssValue value) {
    ...
    for (int i = 0; i < valueElements.size(); i++) {
      if (!(valueElements.get(i) instanceof Identifier) && !(integerValidator.isValid(valueElements.get(i)))) {
        return false;
      }
      ...
      if (i > 0) {
        ...
        if (integerValidator.isValid(valueElements.get(i)) &&
          (!(valueElements.get(i - 1) instanceof Identifier) || noneValidator.isValid(valueElements.get(i - 1)))) {
          return false;
        }
      }
    }
